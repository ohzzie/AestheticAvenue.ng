...
from flask import (
    ...
    flash, abort, send_from_directory, make_response, jsonify,
)
...
def admin_products_import():
    ...
    try:
        import csv, io
        buf = io.StringIO(file.stream.read().decode('utf-8', errors='ignore'))
        reader = csv.reader(buf)
        rows = list(reader)
        if not rows:
            flash("CSV appears empty.", "warning")
            return redirect(url_for("admin_products"))
        # Expect header: title, description, price, image_urls, tags
        header = [h.strip().lower() for h in rows[0]]
        # Best-effort mapping
        def idx(name):
            try: return header.index(name)
            except ValueError: return -1
        i_title = idx('title'); i_desc = idx('description'); i_price = idx('price'); i_imgs = idx('image_urls'); i_tags = idx('tags')
        if min(i_title, i_desc, i_price, i_imgs, i_tags) < 0:
            flash("CSV must include: title, description, price, image_urls, tags.", "danger")
            return redirect(url_for("admin_products"))
        conn = get_db(); c = conn.cursor(); created = 0; skipped = 0
        for r in rows[1:]:
            try:
                title = (r[i_title] or '').strip()
                if not title:
                    skipped += 1; continue
                description = (r[i_desc] or '').strip()
                price_str = (r[i_price] or '').replace(',', '').strip()
                price_cents = int(round(float(price_str)*100)) if price_str else 0
                image_urls = (r[i_imgs] or '').strip()
                tags = (r[i_tags] or '').strip().lower()
                slug = slugify(title)
                now = today_str()
                c.execute(
                    """
                    INSERT OR IGNORE INTO products (title, slug, description, price_cents, image_urls, tags, stock_qty, track_inventory, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?)
                    """,
                    (title, slug, description, price_cents, image_urls, tags, now, now)
                )
                # If slug exists, try appending random suffix
                if c.rowcount == 0:
                    slug = slugify(f"{title}-{uuid.uuid4().hex[:4]}")
                    c.execute(
                        """
                ...
                skipped += 1
        conn.commit(); conn.close()
        msg = f"Imported {created} product(s)." + (f" Skipped {skipped}." if skipped else "")
        flash(msg, "success")
    except Exception:
...
def admin_variant_new(pid):
    ...
    validate_csrf()
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT id FROM products WHERE id=?", (pid,)); base = c.fetchone()
    if not base:
    ...
    price_cents = None
    if price_raw:
        try: price_cents = int(round(float(price_raw)*100))
        except Exception: price_cents = None
    stock_qty = request.form.get('stock_qty', type=int, default=0)
    track_inventory = 1 if request.form.get('track_inventory') else 0
    image_urls = (request.form.get('image_urls') or '').strip()
    now = today_str()
    if not name:
        conn.close(); flash('Variant name is required.', 'danger'); return redirect(url_for('admin_product_edit', pid=pid))
    c.execute(
        """
...
def admin_orders_export():
    ...
    header = [
        ...
        "status","total_cents","created_at","completed_at","receipt_path"
    ]
    rows.append(",".join(header))
    for o in orders:
        vals = [
            o["order_number"], o["customer_name"], o["email"], o["phone"], o["address"],
            o["status"], str(o["total_cents"]), o["created_at"], str(o["completed_at"] or ""), str(o["receipt_path"] or "")
        ]
        rows.append(",".join('"%s"' % str(v).replace('"','""') for v in vals))
    resp = make_response("\n".join(rows))
    fname = f"orders_{status or 'all'}.csv"
...
def admin_order_detail(order_id):
    ...
    if not order:
        conn.close(); abort(404)
    c.execute("SELECT * FROM order_items WHERE order_id=? ORDER BY id", (order_id,)); items = c.fetchall()
    # compute total from items to ensure accurate display
    computed_total = 0
    for i in items:
        computed_total += int(i["price_cents"]) * int(i["quantity"])
    conn.close()
    return render_template("admin_orders.html", detail=True, order=order, items=items, computed_total=computed_total, page_title=f"Order {order['order_number']}")

@app.route("/admin/orders/<int:order_id>/confirm", methods=["POST"])
def admin_order_confirm_payment(order_id):
    redir = _admin_required()
    if redir: return redir
    validate_csrf()
    conn = get_db(); c = conn.cursor()
    c.execute("UPDATE orders SET status='payment_confirmed' WHERE id=?", (order_id,))
    conn.commit(); conn.close()
    flash("Order marked as payment confirmed.", "success")
...
def admin_order_complete(order_id):
    ...
    validate_csrf()
    conn = get_db(); c = conn.cursor()
    c.execute("UPDATE orders SET status='completed', completed_at=? WHERE id=?", (today_str(), order_id))
    conn.commit(); conn.close()
...
def collections_data():
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT id, title, image_urls, tags, created_at FROM products")
    products = c.fetchall(); conn.close()

    tag_map = {}
    def first_image(urls):
        try:
            return [u.strip() for u in (urls or '').split(',') if u.strip()][0]
        except Exception:
            return ""
    ...
    for p in products:
        ...
        for t in [x.strip().lower() for x in raw.split(",") if x.strip()]:
            rec = tag_map.setdefault(t, {"slug": t, "title": t.replace("-"," ").title(), "itemCount": 0, "image": "", "_created_at": ""})
            rec["itemCount"] += 1
            img = first_image(p["image_urls"]) or rec["image"]
            if img and (not rec["image"] or (p["created_at"] or "") > rec["_created_at"]):
                rec["image"] = img
                rec["_created_at"] = p["created_at"] or ""

    overrides_path = os.path.join(os.path.dirname(__file__), 'static', 'data', 'collections_overrides.json')
    overrides = {}
    try:
    ...
    out = []
    for slug, rec in tag_map.items():
        o = overrides.get(slug, {})
        out.append({
            "id": slug,
            "slug": slug,
            "title": o.get("title") or rec["title"],
            "description": o.get("description") or "",
            "image": o.get("image") or rec["image"],
